...
function manageCardsMenu(client: BankClient): void {
  const cardMenuOptions = {
    ...
    '4': () => {
      const cardNumber = readlineSync.question('Ingrese el número de tarjeta a eliminar: ');
    ...
    '5': () => {
      ...
      if (type !== -1) {
        ...
        console.log(`Tarjeta ${newCard.type} solicitada con número: ${newCard.number}`);
      }
    },
    '6': () => console.log('Volviendo al menú principal...')
  };
...
function executeMenu(menuTitle: string, menuOptions: { [key: string]: () => void }, menuDescriptions: { [key: string]: string }, client: BankClient): void {
  ...
  do {
    ...
    Object.keys(menuOptions).forEach(key => {
      console.log(`${key}. ${menuDescriptions[key] || 'Opción desconocida'}`);
    });
    option = readlineSync.question('Seleccione una opción: ');
    (menuOptions[option] || (() => console.log('Opción no válida.')))();
